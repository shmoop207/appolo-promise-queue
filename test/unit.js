...
describe("Promise Queue", function () {
    ...
    it('should run promises with queue concurrency ', async () => {
        let queue = index_1.createQueue({ concurrency: 2 });
        let promise = queue.add(async () => 1);
    ...
    it('should run promises with queue start stop ', async () => {
        let queue = index_1.createQueue({ concurrency: 2 });
        queue.stop();
        queue.add(async () => appolo_utils_1.Promises.delay(20000));
        queue.add(async () => appolo_utils_1.Promises.delay(20000));
        queue.add(async () => appolo_utils_1.Promises.delay(20000));
        queue.add(async () => appolo_utils_1.Promises.delay(20000));
        queue.add(async () => appolo_utils_1.Promises.delay(20000));
    ...
    it('should run promises with queue priority ', async () => {
        const result = [];
        let queue = index_1.createQueue({ concurrency: 1 });
    ...
    it('should handle big queue and concurrency', async () => {
        const concurrency = 5;
        const queue = new index_1.Queue({ concurrency });
        let running = 0;
        const input = new Array(100).fill(0).map(async () => queue.add(async () => {
            ...
            await appolo_utils_1.Promises.delay(appolo_utils_1.Numbers.random(5, 20));
            running--;
        }));
    ...
    it('should run queue with timeout', async () => {
        const result = [];
        const queue = new index_1.Queue({ concurrency: 2, timeout: 10 });
        queue.add(async () => {
    ...
    it('should run queue with onIdle', async () => {
        const queue = new index_1.Queue({ concurrency: 2 });
        queue.add(async () => appolo_utils_1.Promises.delay(100));
        queue.add(async () => appolo_utils_1.Promises.delay(100));
        queue.add(async () => appolo_utils_1.Promises.delay(100));
        queue.size.should.be.eq(1);
        queue.pending.should.be.eq(2);
